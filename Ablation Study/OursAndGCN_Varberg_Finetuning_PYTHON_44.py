...
class AdapterModule(nn.Module):
    ...
    def forward(self, x):
        ...
        x_attention = self.attention(x_reduced)
...
class CustomTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward, dropout):
        ...
        self.adapter = AdapterModule(input_size=d_model, output_size=d_model, bottleneck_size=2)
...
def do_da_test(station, random_seed=42, subsample=1, epochs=20, verbose=True):
    ...
    for name, param in model.named_parameters():
        if 'adapter' in name or 'GCNA' in name:
    ...
    optimizer = optim.Adam(model.parameters(), lr=learning_rate, weight_decay=1e-4)  # Add L2 regularization with weight_decay
...
if __name__ == "__main__":
    best_model, train_losses, val_losses, best_epoch = do_da_test("varnamo")
