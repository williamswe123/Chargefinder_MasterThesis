...
class CrossModalAttention(nn.Module):
    def __init__(self, feature_dim, bottleneck_size):
        super(CrossModalAttention, self).__init__()
        self.query = nn.Linear(feature_dim, bottleneck_size)
        self.key = nn.Linear(feature_dim, bottleneck_size)
        self.value = nn.Linear(feature_dim, bottleneck_size)
        
        self.expand = nn.Linear(bottleneck_size, feature_dim)
    ...
    def forward(self, x):
        ...
        return self.expand(torch.matmul(attention, V))

class AdapterModule(nn.Module):
    def __init__(self, input_size, output_size, bottleneck_size, dropout_rate=0.1):
        super(AdapterModule, self).__init__()
        self.attention = CrossModalAttention(input_size, bottleneck_size)
        
    def forward(self, x):
        x_expanded = self.attention(x)
...
class CustomTransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward, dropout):
        ...
        self.adapter = AdapterModule(input_size=d_model, output_size=d_model, bottleneck_size=1)
...
def do_da_test(station, random_seed=42, subsample=1, epochs=20, verbose=True):
    ...
    original_state_dict = torch.load('Transfer Learning/trained_on_varberg44.pth')
    ...
    # Unfreeze adapter and decoder parameters
    for name, param in model.named_parameters():
        if 'adapter' in name or 'transformer_decoder' in name:
...
if __name__ == "__main__":
    best_model, train_losses, val_losses, best_epoch = do_da_test("malmo")
