...
def do_da_test(pretrain_station="varnamo", finetune_station="varberg", pretrain_seed=42, finetune_seed=42, subsample=1, epochs = 20, verbose=True):
    ...
    # Use the function
    train_loader, val_loader, test_loader = getLoader(station=finetune_station, future_steps=future_steps,
                                                      seq_len=seq_len, batch_size=batch_size,
                                                      random_seed=finetune_seed, subsample=subsample)
...
if __name__ == "__main__":
    ...
    best_model, train_losses, val_losses, test_losses, best_epoch = do_da_test(subsample=0.01)
    
    torch.save(best_model.state_dict(), "Transfer Learning/trained_on_{}-finetuned_on_varberg.pth")
